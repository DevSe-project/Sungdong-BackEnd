...
const estimateController = {
  ...
  filter: async (req: Request, res: Response) => {
    ...
    try {
      ...
      const requestData = req.body;
      const newFilter = {
        dateStart: requestData.start,
        dateEnd: requestData.end
      }
      Estimate.filter(req.user.users_id, newFilter, currentPage, postsPerPage, (err: { message: any; }, data: ResultSetHeader | RowDataPacket | RowDataPacket[] | null) => {
        // 클라이언트에서 보낸 JSON 데이터를 받음
        if (err)
