...
class Rae {
      ...
      function executeQuery(queryIndex: number) {
        if (queryIndex < queries.length) {
          ...
          if (
            ...
            connection.query(
              query,
              [newProduct[`product${queryIndex + 1}`]],
              (err, res) => {
                if (err) {
                  console.log(
                    `쿼리 실행 중 에러 발생 (인덱스 ${queryIndex}): `,
  ...
  static list(
    ...
    const query =
      "SELECT * FROM rae JOIN rae_product ON rae.rae_id = rae_product.rae_id JOIN order_product ON order_product.order_product_id = rae_product.order_product_id JOIN product ON product.product_id = order_product.product_id WHERE rae.users_id = ? ORDER BY rae.rae_requestDate DESC LIMIT ?, ?";
    // 전체 데이터 크기 확인을 위한 쿼리
    const countQuery =
      "SELECT COUNT(*) as totalRows FROM rae JOIN rae_product ON rae.rae_id = rae_product.rae_id WHERE rae.users_id = ?";
  ...
  static adminModule(
    ...
    const query = `
    SELECT 
      (SELECT COUNT(*) FROM rae JOIN rae_product ON rae.rae_id = rae_product.rae_id WHERE rae_product.rae_type = 0 AND rae.raeState = 1) AS refund,
      (SELECT COUNT(*) FROM rae JOIN rae_product ON rae.rae_id = rae_product.rae_id WHERE rae_product.rae_type = 1 AND rae.raeState = 1) AS exchange,
      (SELECT COUNT(*) FROM \`order\` WHERE order.orderState = 6) AS cancel`;
      connection.query(
        query,
        (err: QueryError | null, res: RowDataPacket[]) => {
          if (err) {
  ...
  static selectProductById(
    order_id: any,
    result: (arg0: any, arg1: any) => void
  ) {
    const query =
      "SELECT * FROM rae JOIN rae_product ON rae.rae_id = rae_product.rae_id JOIN order_product ON order_product.order_product_id = rae_product.order_product_id JOIN product ON order_product.product_id = product.product_id WHERE rae.rae_id IN (?)";
    connection.query(
      query,
      [order_id],
      (err: QueryError | null, res: RowDataPacket[]) => {
  ...
  static changeStatusbyId(obj: any, result: (arg0: any, arg1: any) => void) {
    const query =
      "UPDATE rae SET raeState = ?, rae_checkDate = ?,rae_cancelReason = NULL WHERE rae_id = ?";
    const promises = obj.map((item: any) => {
      return new Promise((resolve, reject) => {
        connection.query(
          query,
          [item.raeState, new Date(), item.rae_id],
          (err, res) => {
            if (err) {
              reject(err);
            } else {
  ...
  static changeCancelbyId(obj: any, result: (arg0: any, arg1: any) => void) {
    const query =
      "UPDATE rae SET raeState = ?, rae_cancelReason = ?, rae_checkDate = ? WHERE rae_id = ?";
    const promises = obj.map((item: any) => {
      return new Promise((resolve, reject) => {
        connection.query(
          query,
          [item.raeState, item.rae_cancelReason, new Date(), item.rae_id],
          (err, res) => {
            if (err) {
              reject(err);
            } else {
  ...
  static filter(
    ...
    if (newFilter.dateStart !== "" && newFilter.dateEnd !== "") {
      conditions.push(
        `${newFilter.raeDateType} BETWEEN '${newFilter.dateStart} 00:00:00' AND '${newFilter.dateEnd} 23:59:59'`
      );
    }
  ...
  static raeFilter(
    ...
    const conditions = conditionColumns.filter(
      (column) => newFilter[column.split(".")[1]] !== undefined
    );
    const conditionString =
      conditions.length > 0
        ? "AND " +
          conditions.map((condition) => condition + " LIKE ?").join(" AND ")
        : "";

    const dateCondition =
      newFilter.raeDateType !== "" &&
      newFilter.dateStart !== "" &&
      newFilter.dateEnd !== ""
    ...
    const typeCondition =
      newFilter.rae_type !== "" ? `AND rp.rae_type = ?` : "";
    const stateCondition =
      newFilter.raeState !== "" ? `AND r.raeState = ?` : "";
  ...
  static deleteByIds(
    product: number[],
    result: (error: any, response: any) => void
  ) {
